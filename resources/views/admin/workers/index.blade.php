...
@section('page-subtitle', 'Manage all gym staff and workers')

@section('content')

<div class="flex justify-between items-center mb-6">
    <div></div>
    {{-- Block Add Button for Super Admin --}}
    ...
    <table class="w-full">
        <thead>
            <tr class="bg-gray-50 border-b border-gray-200">
                <th class="text-left px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Worker</th>
        ...
        <tbody>
            ...
            <tr class="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                <td class="px-5 py-4">
                    <div class="font-semibold text-gray-900">{{ $worker->first_name }} {{ $worker->last_name }}</div>
                    <div class="text-xs text-gray-400 mt-0.5">{{ $worker->email ?? '—' }}</div>
                    <div class="text-xs text-gray-400">{{ $worker->phone ?? '—' }}</div>
                </td>
                <td class="px-5 py-4 text-sm text-gray-600">{{ $worker->position }}</td>
                <td class="px-5 py-4 text-sm text-gray-600">{{ $worker->branch?->branch_name ?? '—' }}</td>
                <td class="px-5 py-4 text-sm text-gray-500">{{ $worker->date_hired?->format('M d, Y') ?? '—' }}</td>
                <td class="px-5 py-4">
                ...
                <td class="px-5 py-4">
                    <div class="flex items-center gap-2">
                        <a href="{{ route('admin.workers.show', $worker->worker_id) }}"
                        ...
                        {{-- Block Edit and Delete for Super Admin --}}
                        @if(Auth::user()->isBranchAdmin())
            ...
            @empty
            <tr>
                <td colspan="6" class="px-5 py-12 text-center text-gray-400">No workers found.</td>
            </tr>
            @endforelse
        </tbody>
    </table>
    @if($workers->hasPages())
    <div class="px-5 py-4 border-t border-gray-200">{{ $workers->links() }}</div>
    @endif
</div>
