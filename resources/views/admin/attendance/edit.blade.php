...
<div class="min-h-screen bg-gray-50">
    <div class="max-w-2xl mx-auto py-6 sm:px-6 lg:px-8">
        ...
        <div class="mb-6">
            ...
            <h1 class="text-3xl font-bold text-gray-900 mt-2">Edit Check-In Record</h1>
            <p class="mt-2 text-sm text-gray-600">Modify member attendance details</p>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            <form action="{{ route('admin.attendance.update', $attendance->attendance_id) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <select id="branch_id" name="branch_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm border px-3 py-2" required onchange="loadMembers()">
                        ...
                        <option value="{{ $branch->branch_id }}" {{ old('branch_id', $attendance->branch_id) == $branch->branch_id ? 'selected' : '' }}>
                            {{ $branch->branch_name }}
                        </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select id="member_id" name="member_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm border px-3 py-2" required>
                        ...
                        <option value="{{ $attendance->member_id }}" selected>
                ...
                <div>
                    ...
                    <input type="date" id="check_in_date" name="check_in_date" value="{{ old('check_in_date', $attendance->check_in_date) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm border px-3 py-2" required>
                    @error('check_in_date')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="time" id="check_in_time" name="check_in_time" value="{{ old('check_in_time', $attendance->check_in_time) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm border px-3 py-2" required>
                    @error('check_in_time')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
...
<script>
    function loadMembers() {
        ...
        fetch(`{{ route('admin.attendance.getMembersByBranch') }}?branch_id=${branchId}`)
            ...
            .then(data => {
                const currentMemberId = '{{ $attendance->member_id }}';
                memberSelect.innerHTML = '<option value="">-- Select a member --</option>';
                data.forEach(member => {
